...
/* $Id: parsecfg.c,v 1.1.1.1 2010-01-21 17:22:42 rocco Exp $ */
...
void cfgFreeList (cfgList * item)
{
  if (! item)
    return;
  cfgFreeList (item -> next);
  free (item -> str);
  free (item);
}
...
int cfgSectionNameToNumber(const char *name)
{
	...
	for (i = 0; i < parsecfg_maximum_section; i++) {
		if (strcmp(name, parsecfg_section_name[i]) == 0) {
			return (i);
		}
	}
	return (-1);
...
static int store_value(cfgStruct cfg[], const char *parameter, const char *value, cfgFileType type, int section)
{
	...
	for (num = 0; cfg[num].type != CFG_END; num++) {
		if (strcmp(parameter, cfg[num].parameterName) == 0) {
			/*
...
static int parse_ini(const char *file, FILE *fp, char *ptr, cfgStruct cfg[], int *line, int *section)
{
	...
	if (*ptr == '[') {
		...
		for (i = 0; i < *section; i++) {
			if (strcmp(parsecfg_section_name[*section], parsecfg_section_name[i]) == 0) {
	...
	while ((ptr = get_single_line_without_first_spaces(fp, &line_buf, &line)) != NULL) {	/* malloc line_buf */
		if (*ptr == '[') {
			...
			if ((section_num > 0 && current_section_number == section_num) ||
			    (section_num <= 0 && strcmp(read_section_name, section_name) == 0)) {
				section_flag = 0;
				free(line_buf);
				free(read_section_name);
				continue;
		...
		/* Return the first value in case of duplicated entries */
		if (store_flag == 0 && value_type != CFG_STRING_LIST)
		  break;
...
static int fetch_value(const char *file, int *line, char *line_buf, FILE *fp, int *store_flag, char *ptr, cfgStruct *fetch_cfg)
{
	...
	if (strcmp(read_parameter, fetch_cfg[0].parameterName) == 0) {
		if (*ptr == '{') {
			ptr = rm_first_spaces(ptr + 1);
			if (*ptr != '\0' && *ptr != '#') {
				cfgFatal(CFG_SYNTAX_ERROR, file, *line, line_buf);
